...
require_once(root_xrcommon_adm.'checkright_upload.php');
...
require(dirname(__FILE__).'/shearphoto.config.php');
...
class ShearPhoto
{
    ...
    protected function CreateArray(&$PHPconfig, &$JSconfig)
    {
        ...
        foreach ($PHPconfig["width"] as $k => $v) {
            ...
            $arr[$k] = array($v[0], $height, $PHPconfig["saveURL"] . $suffix . $PHPconfig["filename"], $v[1] === true and $water_or === true and $v[0] > $PHPconfig["water_scope"] and $height > $PHPconfig["water_scope"]);
        }
...
$aliresult = aliyun_ossupdate($web_url,$arr_p[0],$result[0]['ImgName']);
