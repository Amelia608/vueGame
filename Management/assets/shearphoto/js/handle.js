...
window.ShearPhoto.MINGGE(function() {
	var b, c, d, e, f, g, h, i, k, l, m, n, p, q, r, s, t, u, v, w, x, y, a = "shearphoto",shear_w;
	...
	a = a.replace(/(^\s*)|(\s*$)/g, ""), "" === a || (a += "../../"), b = document.getElementById("relat"), c = b.getElementsByTagName("img"), d = new ShearPhoto, d.config({
		...
		HTML5ZIP: [1200, 1],
		preview: false,
		url: a + "php/shearphoto.php?ac=" + strs[0],
		scopeWidth: 300,
		scopeHeight: 200,
		proportional: [strs[1] / strs[2], shear_w, strs[2]],
		Min: 50,
		Max: 200,
		backgroundColor: "#000",
		backgroundOpacity: .6,
		...
		this.HTML5.EffectsReturn(), this.HTML5.BOLBID && this.HTML5.URL.revokeObjectURL(this.HTML5.BOLBID), e = document.createElement("DIV"), e.className = "completeTxt", e.innerHTML = "<strong><i></i>图片上传成功</strong> <button id='completeA'>上传成功</button>", c.appendChild(e), g = document.getElementById("completeA"), h = this, h.preview.close_(), g.onclick || (g.onclick = function() {
			$('#img_iframe'+strs[0], window.parent.document).attr("class", "hidden");
			$('#image_block'+strs[0], window.parent.document).attr({"class":"xr_uploadimg_thumn",'src':a[a.length-1]['ImgUrl']});
			$('#image_title'+strs[0], window.parent.document).attr('value', a[0]['ImgName']);
			$('#reuplod'+strs[0], window.parent.document).attr("class", "btn btn-sm show img_upload_reuplod")
		})
