...
class mysql{
    ...
    function connect() {
        ...
        $db_name     = "bmw_vote";
        $dbcharset   = "utf8";
        $this->$link = mysqli_connect ($db_server,$db_username,$db_password) or die ('数据库连接失败');
        mysqli_set_charset ($this->$link,$dbcharset);
        mysqli_select_db ($this->$link,$db_name) or die ( '指定数据库打开失败<br/>ERROR ' . mysqli_errno ($this->$link ) . ':' . mysqli_error ($this->$link) );
        return$this->$link;
    }
