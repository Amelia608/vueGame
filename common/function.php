 ...
 * @return [type] [description]
 */
function get_image_width($filePath){
	$image = null;
	$extension = strtolower(preg_replace('/^.*\./', '', $filePath));
	switch($extension){
		case 'jpg':
		case 'jpeg':
 ...
 * @return [type] [description]
 */
function get_image_height($filePath){
	$image = null;
	$extension = strtolower(preg_replace('/^.*\./', '', $filePath));
	switch($extension){
		case 'jpg':
...
//格式化日前成中文的
function show_date_cn($datestr,$showtype){
	$y=date("Y",$datestr);
	$m=date("m",$datestr);
	$d=date("d",$datestr);
	$h=date("H",$datestr);
 ...
 * @param  array $post_fields [description]
 *
 * @return [type] [description]
 */
function simulate_post($url,$post_fields=array()){
    $ch=curl_init();
    curl_setopt($ch,CURLOPT_URL,$url);
    curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
 ...
 * @return [type] [description]
 */
function aliyun_ossupdate($web_url,$filePath,$filename){
	// if ($web_url=="http://cn-autosalon.com") {
		unset($post_arr);
		$post_arr['type'] = str_replace("\\","",str_replace("/","",$filePath));
		$post_arr['name'] = $filename;
		// $oss_update = simulate_post($web_url."/assets/aliyun_oss/Object.php",$post_arr);
		return $oss_update;
	// }
}
 ...
 * @return [type] [description]
 */
function get_imagepath($level,$path){
	global $config;
	$image_Path = "";
	if ($config['cas_aliyunosslevel']==0) {
		if ($level==3) {
			$image_Path = $config['dir_base_path'].$config[$path]."/small_";
		}else{
			$image_Path = $config['dir_base_path'].$config[$path]."/";
		}
	}else{
		if ($level <= $config['cas_aliyunosslevel']) {
			$image_Path = $config['ali_oss_url'].$config[$path]."-";
		}else{
			if ($level==3) {
				$image_Path = $config['dir_base_path'].$config[$path]."/small_";
			}else{
